...
import { CONFIG, getFireTypeConfig, getPowerUpMultiplier, addPlayerScore, getPowerUpGraphicFilename } from '../config.js';
...
export class BurningVaultSystem {
  ...
  _trySpawnOnePoolRewardAt(q, r, selected) {
    ...
    if (type === 'water_tank') {
      const id = this.gameState.waterTankSystem?.spawnWaterTank(q, r);
      return { ok: !!id, preview: id ? itemPreview('water_tank.png') : null };
    }
  ...
  _spawnPoolRewardOnVaultHex(q, r, selected) {
    ...
    const { ok, preview } = this._trySpawnOnePoolRewardAt(q, r, selected);
    if (ok && preview?.spriteFilename) {
      this.gameState.notificationSystem?.addBurningVaultRewardSpawnPreview?.(q, r, preview);
    }
  }
