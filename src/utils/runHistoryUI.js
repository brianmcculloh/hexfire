...
function renderDetail(entry) {
  ...
  const fires = s.firesExtinguishedByType || {};
  ...
  listEl.innerHTML = entries
    .map((entry, i) => {
      ...
      <button type="button" class="run-history-list-item" data-entry-index="${idx}">
        ...
        <span class="run-history-list-meta">${outcomeLabel(entry.outcome)} · Wave ${escapeHtml(String(wave))} · ${escapeHtml(score)} pts</span>
      </button>`;
    })
    .join('');
