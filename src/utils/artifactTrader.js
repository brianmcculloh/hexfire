...
function playerOwnsArtifactId(gameState, artifactId) {
  ...
  return col.some((id) => String(id) === s);
}
...
function artifactImgElement(artifactId, maxWidthPx) {
  ...
  img.style.cssText = `height: ${TRADER_ARTIFACT_IMG_HEIGHT_PX}px; width: auto; max-width: ${mw}px; min-width: 0; object-fit: contain; image-rendering: pixelated; cursor: inherit; flex-shrink: 1;`;
  return img;
}
  ...
  for (const part of rewardBundle) {
    ...
    if (part.type === 'currency') {
      ...
      if (total > 0) {
        ...
        icon.style.cssText = `height: ${TRADER_REWARD_ICON_H_PX}px; width: auto; image-rendering: pixelated; flex-shrink: 0; cursor: inherit;`;
    ...
    for (let u = 0; u < n; u++) {
      if (part.type === 'upgrade_plans') {
        ...
        img.style.cssText = `height: ${TRADER_REWARD_ICON_H_PX}px; width: auto; image-rendering: pixelated; flex-shrink: 0; cursor: inherit;`;
...
function wireTraderItemTooltips(root, gameState) {
  root.querySelectorAll('img[data-artifact-id]').forEach((img) => {
    ...
    img.addEventListener('mouseenter', (e) => {
      ...
      if (!ts) return;
      const html = ts.getArtifactTooltipContentForInventory(id);
...
export function mountArtifactTraderModalBody(statsDiv, wants, gameState, opts = {}) {
  ...
  wrap.style.cssText = 'max-width: 920px; margin: 0 auto; text-align: center;';
  ...
  // The wrap's effective width is forced to its `max-width: 920px` ceiling because the Continue
  // button row below uses a `width: 100vw` full-bleed trick. That makes wrap wider than the modal
  // frame and the orange header, so a blurb that just relied on `text-align: center` inside wrap
  // would visually drift off-center relative to the header. Give the blurb its own bounded,
  // auto-centered width so it stays visually centered under "Artifact Trader" no matter how wide
  // the surrounding wrap or button row gets.
  blurb.style.cssText =
  ...
  columns.style.cssText =
    'display: flex; flex-direction: row; flex-wrap: nowrap; justify-content: center; align-items: flex-start; gap: 22px 82px;';
  ...
  if (btn) {
    ...
    // Full-bleed row inside centered modal content: snap to viewport edges, then center button.
    btnWrap.style.cssText = 'position: relative; left: 50%; right: 50%; margin-left: -50vw; margin-right: -50vw; width: 100vw; display: flex; justify-content: center; margin-top: 16px; padding-bottom: 4px; z-index: 4;';
