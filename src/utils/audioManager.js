...
/** Effective music gain for a track key (group30-loop is boosted for the finale). */
function getMusicGainForKey(musicKey) {
  const base = getBaseMusicEffectiveGain();
  const mult = musicKey === 'group30' ? GROUP_30_LOOP_VOLUME_MULTIPLIER : 1;
  return Math.min(1, base * mult);
}

/** Apply music gain from slider + current track (group 30 finale boost). */
