...
export class NotificationSystem {
  ...
  /**
   * Floating combo flourish (same motion as XP text, larger type).
   * @param {number} q - Hex q (may be fractional for group centroid)
   * @param {number} r - Hex r (may be fractional for group centroid)
   * @param {string} text - Combo phrase
   * @param {string} color - Hex color for the phrase and bonus XP line
   * @param {number} [xp] - Boosted combo XP awarded (shown under the phrase)
   */
  addComboNotification(q, r, text, color, xp = 0) {
    const displayText = String(text || '').replace(/\b\w/g, (c) => c.toUpperCase());
    const xpAmount = Math.max(0, Math.round(Number(xp)) || 0);
    this.notifications.push({
      id: this.nextId++,
      q,
      r,
      ...
      color: color || '#00F0FF',
      life: 0,
      maxLife: CONFIG.COMBO_FLOAT_DURATION_SEC ?? 2.0,
      isCombo: true,
      comboXpText: xpAmount > 0 ? `+${xpAmount.toLocaleString()} XP` : '',
    });
  }
